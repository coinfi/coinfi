...
const Social = (props: IProps) => {
  ...
  const isRedditSelected = props.feedSources.includes('reddit')
  const isTwitterSelected = props.feedSources.includes('twitter')
  ...
  return (
    <>
      ...
      <Block label="Reddit" logo={RedditLogo}>
        <Switch on={isRedditSelected} onChange={props.onToggleReddit} />
      </Block>
      <Block label="Twitter" logo={TwitterLogo}>
        <Switch on={isTwitterSelected} onChange={props.onToggleTwitter} />
      </Block>
    </>
  )
}
